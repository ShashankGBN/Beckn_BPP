...
public class UpdateService implements ProcessInternalRequestService {
	...
	public void processCallbackRequest() {
		...
		for (ServiceRequestFlow f : flows) {
			...
			StateEnum orderState = getStatus(confirmBody);

			System.out.println("State of order --------------" + orderState);
	...
	public StateEnum getStatus(ConfirmBody confirmBody) {

		return confirmBody.getMessage().getOrder().getState();
	}
	...
	public OnConfirmBody getOnConfirmBody(ConfirmBody confirmBody) {
		...
		onConfirmBody.getMessage().setOrder(new Order().id("order_id-001").state(StateEnum.CANCELLED).provider(new Provider().id("Tibil solutions")));

		Item items = new Item().id("EMP-200");
		...
		onConfirmBody.getMessage().setOrder(new Order().quote(new Quotation().price(new Price().currency("INR").value("30"))));
